"""
metrics.py
Attack-effectiveness metrics: how strongly, and how selectively, TimeCAT's
adversarial perturbation moved the surrogate's forecast for its target
channel versus every other channel. Ported unchanged from TimeCAT_new's
metrics.py -- these are used by utils.attack_utils.run_condition and, in
aggregate, by analyze_results.py.

Not to be confused with the structure-recovery metrics (AUROC/F1/SHD of an
estimated causal graph against ground truth) that the sibling
codebase_caufrts project's metric.py computes -- this is a different
question (how effective/selective was one attack run) for a different kind
of model (a frozen causally-masked forecaster being attacked, not a
causal-discovery model being trained).
"""
...
def compute_css(pred_clean, pred_adv, target_dim, gamma=1e-8):
    ...
    CSS = ||f(X_adv)^(j) - f(X)^(j)||_2 / (||f(X_adv)^(-j) - f(X)^(-j)||_2 + gamma)
    """
...
def compute_mse_tgt(pred_adv, ground_truth, target_dim):
    """
    MSEtgt = (1/H) sum (f(X_adv)^(j)_h - Y^(j)_h)^2
...
def compute_mae_tgt(pred_adv, ground_truth, target_dim):
    """
    MAEtgt = (1/H) sum |f(X_adv)^(j)_h - Y^(j)_h|
...
def compute_mse_nt(pred_clean, pred_adv, target_dim):
    """
    MSEnt = (1/(H*(M-1))) sum_{k!=j} sum_h (f(X_adv)^(k)_h - f(X)^(k)_h)^2
...
def compute_mae_nt(pred_clean, pred_adv, target_dim):
    """
    MAEnt = (1/(H*(M-1))) sum_{k!=j} sum_h |f(X_adv)^(k)_h - f(X)^(k)_h|
...
def compute_pe(pred_clean, pred_adv, delta, target_dim, gamma=1e-8):
    """
    PE = ||f(X_adv)^(j) - f(X)^(j)||_2 / (||delta*||_F + gamma)
